...
- [Commands](#commands)
  - [Simple Account](#simple-account)
    ...
    - [Transfer ERC-20 token](#transfer-erc-721-token)
...
```bash
yarn run simpleAccount erc721Transfer --token <address> --id <tokenId> --to <address>
```

### Fulfill top Opensea offer

Make sure the address generated by `simpleAccount:address` has enough specified tokens to execute the transfer.

If not using a paymaster, make sure to also have enough ETH to pay gas fees.

Opensea url should be like: https://testnets.opensea.io/assets/goerli/0xbca3eb0c54f68ca559671906c65e2837991848bc/3843

```bash
yarn run simpleAccount seaportFulfillOrder --url <opensea_url>
```
